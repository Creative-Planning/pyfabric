...
def main(argv: list[str] | None = None) -> int:
    ...
    if not argv or argv[0] in ("-h", "--help"):
        print(
            ...
            "\n"
            "Usage:\n"
            "  pyfabric install-claude-memory [--project [PATH]] [--force] [--dry-run]\n"
            "      Install pyfabric's reference memories into your active\n"
            "      Claude profile (global by default, so every Claude session\n"
            "      picks them up). Use --project to scope to a single repo.\n"
            "\n"
